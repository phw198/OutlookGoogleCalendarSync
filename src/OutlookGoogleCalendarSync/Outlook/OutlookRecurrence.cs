...
namespace OutlookGoogleCalendarSync.Outlook {
    public class Recurrence {
        ...
        private static void buildOutlookPattern(Event ev, AppointmentItem ai, out RecurrencePattern oPattern) {
            ...
            switch (ruleBook["FREQ"]) {
                ...
                case "MONTHLY": {
                        ...
                        if (ruleBook.ContainsKey("BYSETPOS")) {
                            ...
                            if (oPattern.DayOfWeekMask == (OlDaysOfWeek)127 && gInstance == -1 && ev.Start.SafeDateTimeOffset(!ev.AllDayEvent()).Day > 28) {
            ...
            oPattern.PatternStartDate = ev.Start.SafeDateTimeOffset(!ev.AllDayEvent()).DateTime;
        ...
        public static void CompareOutlookPattern(Event ev, ref RecurrencePattern aiOpattern, Sync.Direction syncDirection, System.Text.StringBuilder sb, ref int itemModified) {
            ...
            evAI.Start = ev.Start.SafeDateTimeOffset(!ev.AllDayEvent()).DateTime;
        ...
        private static int processOutlookExceptions(List<Event> evExceptions, ref AppointmentItem ai, Boolean forceCompare, Boolean processingDeletions) {
            ...
            try {
                ...
                foreach (Event gExcp in evExceptions) {
                    System.DateTime gExcpOrigDate = gExcp.OriginalStartTime.SafeDateTimeOffset(!gExcp.AllDayEvent()).DateTime;
                    System.DateTime? gExcpCurrDate = gExcp.Start?.SafeDateTimeOffset(!gExcp.AllDayEvent()).DateTime;
                    ...
                    try {
                        ...
                        if (gExcp.Status == "cancelled") {
                            ...
                            Outlook.Calendar.Instance.UpdateCalendarEntry(ref newAiExcp, gExcp, ref itemModified,
                                forceCompare || gExcp.Start.SafeDateTimeOffset(!gExcp.AllDayEvent()).Date != newAiExcp.Start.Date);
                            if (itemModified > 0) {
                                try {
                                    newAiExcp.Save();
                                    updatesMade++;
